...
namespace :recommendations do
  ...
  task :rebuild_redis_user, [:user_id] => :environment do |_t, args|
    ...
    async = ENV['ASYNC'] == 'true' || ENV['ASYNC'] == '1'
    
    if user_id.blank?
      puts "Usage: rake recommendations:rebuild_redis_user[USER_ID]"
      puts "   or: USER_ID=123 rake recommendations:rebuild_redis_user"
      puts "   or: ASYNC=true USER_ID=123 rake recommendations:rebuild_redis_user (enqueue Sidekiq job)"
      exit 1
    end
    ...
    if async
      puts "Enqueuing recommendation update job for user #{user.id} (#{user.email})..."
      UpdateRecommendationsJob.perform_later(user.id, resource_types: ['Book', 'Author', 'Category', 'Publisher', 'Shelf'])
      puts "Job enqueued! It will be processed in the background via Sidekiq."
    else
  ...
  task rebuild_redis_all: :environment do
    ...
    if async
      ...
      puts "This will process in the background via Sidekiq..."
  ...
  task rebuild_redis_with_ratings: :environment do
    ...
    if async
      ...
      user_ids.each do |user_id|
        UpdateRecommendationsJob.perform_later(user_id, resource_types: ['Book', 'Author', 'Category', 'Publisher', 'Shelf'])
        count += 1
        print "." if count % 10 == 0
      end
