...
class RecommendationService::CacheManager
  ...
  def self.cache_recommendations(user, resource_type, recommendations, limit:)
    key = cache_key(user, "recommendations", resource_type, limit)
  ...
  def self.get_similar_users(user, limit, resource_type: 'Book')
    key = cache_key(user, "similar_users", resource_type, limit)
    cached_ids = Rails.cache.read(key)
    return nil unless cached_ids.present?
  ...
  # Accepts either ActiveRecord::Relation or Array
  def self.cache_similar_users(user, similar_users, limit, resource_type: 'Book')
    key = cache_key(user, "similar_users", resource_type, limit)
    # Store IDs instead of full objects for efficiency
  ...
  def self.clear_user(user, resource_types: RecommendationService::RESOURCE_TYPES)
    Array(resource_types).each do |resource_type|
      Rails.cache.delete_matched("recommendations/user:#{user.id}/recommendations/#{resource_type.downcase}/*")
      Rails.cache.delete_matched("recommendations/user:#{user.id}/similar_users/#{resource_type.downcase}/*")
    end
  end
